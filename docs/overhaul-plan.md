...
## 2. Phase 1 — Repo hygiene (trivial, do first)
...
## 3. Phase 2 — One mainline, one deploy story (the structural decision)
...
**Recommended model** (minimal moving parts for a solo project):

1. `master` becomes the only long-lived branch.
2. `deploy.yml`: push to `master` → auto-deploy the **dev** environment; **prod** deploys via `workflow_dispatch` (manual button) or a `v*` tag. This kills the merge dance entirely.
3. Resurrect the CI gate from the archived branch (`git show archive/refactor-cicd-deployment:.github/workflows/ci.yml`): npm ci → build shared → type-check → test, with a Postgres service container. Adapt its `branches: [main, dev]` to `master`, wire it as a `workflow_call` prerequisite job in `deploy.yml`, and run it on PRs + pushes to master.
4. After the switch: delete `main` and `dev`, and fold `type-check.yml` into the new CI workflow.
5. Script fixes that support this:
   - Root `type-check` must build shared first: `npm run build -w shared && ...` (fresh-clone failure verified in §0).
   - `dev:setup` uses Windows-only `timeout /t 3` — replace with `node -e "setTimeout(()=>{},3000)"` or drop.
   - `docker:*` scripts use legacy `docker-compose`; deploy uses `docker compose` — standardize on v2.

## 4. Phase 3 — Delete the dead duplicates
...
- **Larger audit (schedule separately):** the crawler/stats subsystem is *live*, but two generations of tables coexist — older periodic snapshots (`snapshots.ts`: `statSnapshots`, leaderboard/map/faction/unit snapshots) alongside newer crawler-derived tables (`crawler.ts`: match results/picks/deployments, spec/unit-performance snapshots). Audit which old tables are still read and retire the rest with a migration.

## 5. Phase 4 — Make the docs tell the truth
...
## 6. Phase 5 — Config & env rationalization
