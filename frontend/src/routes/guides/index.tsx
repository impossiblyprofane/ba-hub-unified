...
export default component$(() => {
  ...
  return (
    <div class="w-full max-w-[2000px] mx-auto">
      <div class="mb-6">
        <p class="text-[var(--accent)] text-xs font-mono tracking-[0.3em] uppercase mb-3">{t(i18n, 'guides.tag')}</p>
        <h1 class="text-3xl font-semibold text-[var(--text)] tracking-tight">{t(i18n, 'guides.title')}</h1>
        <p class="text-sm text-[var(--text-dim)] mt-2 max-w-2xl">{t(i18n, 'guides.subtitle')}</p>
      </div>
      ...
      <div class="p-0 bg-gradient-to-b from-[var(--bg)] to-[rgba(26,26,26,0.7)] border border-[rgba(51,51,51,0.15)]">
        ...
        <div class="px-4 py-12 flex flex-col items-center justify-center gap-3 text-center">
          ...
          <p class="text-sm text-[var(--text-dim)] max-w-lg">{t(i18n, 'guides.comingSoon')}</p>
        </div>
      </div>
    </div>
  );
