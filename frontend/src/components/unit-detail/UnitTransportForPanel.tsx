...
import type { UnitDetailTransportFor } from '~/lib/graphql-types';

type Props = { transportFor: UnitDetailTransportFor; compact?: boolean; fill?: boolean };

/**
 * Reverse-availability panel — shown when the current unit is itself a
 * transport (e.g. Bradley, K-16). Surfaces the specialization the transport
 * belongs to and the parent infantry units that may take it. Mirrors the
 * UnitAvailabilityPanel layout so it slots into the same column visually.
 */
export const UnitTransportForPanel = component$<Props>(({ transportFor, compact, fill }) => {
  const i18n = useI18n();
  const spec = transportFor.specialization;
  ...
  return (
    ...
    >
      ...
      <div class={`flex flex-col ${compact ? 'gap-2' : 'gap-3'} ${fill ? 'flex-1 overflow-y-auto' : ''}`}>
        <div class={`bg-[rgba(26,26,26,0.4)] border border-[rgba(51,51,51,0.1)] ${compact ? 'p-1' : 'p-1.5'}`}>
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-1">
                {transportFor.parentUnits.map(parent => (
                  ...
                  >
                    {parent.ThumbnailFileName && (
                      <img
                        src={toUnitIconPath(parent.ThumbnailFileName)}
                        width={14} height={14}
