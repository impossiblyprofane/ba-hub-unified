/**
 * ReadonlyUnitPanel — readonly version of UnitEditorPanel for the published
 * deck detail page. Same dual-panel layout (main unit + transport) with
 * identity headers, modifications (shown as static labels, not dropdowns),
 * and full stats display (armor, mobility, weapons, traits).
 *
 * No edit controls: no modification dropdowns, no transport selector,
 * no count change. Only onClose$ callback.
 */
import {
  component$, useResource$, useSignal, Resource,
} from '@builder.io/qwik';
import type { PropFunction } from '@builder.io/qwik';
import type { UnitConfig, DeckModification } from '@ba-hub/shared';
import type {
...
interface ReadonlyUnitPanelProps {
  unit: UnitConfig;
...
/** Render modifications as static read-only labels (no dropdowns). */
function renderModsReadonly(
      ...
      <Resource
        ...
        onResolved={(mods) => {
          ...
          return (
            <div class="space-y-2">
              {sorted.map((mod) => {
                const selEntry = modList.find(m => m.modId === mod.Id);
                const selOpt = selEntry
                  ? mod.options.find(o => o.Id === selEntry.optId)
                  : null;
                const label = getGameLocaleValueOrKey(GAME_LOCALES.modopts, mod.UIName || mod.Name, locale) || mod.Name || `Mod ${mod.Id}`;
                const optLabel = selOpt
                  ? getGameLocaleValueOrKey(GAME_LOCALES.modopts, selOpt.UIName || selOpt.Name, locale) || selOpt.Name || `Option ${selOpt.Id}`
                  : '—';
                const cost = selOpt?.Cost && selOpt.Cost > 0 ? ` (+${selOpt.Cost})` : selOpt?.Cost && selOpt.Cost < 0 ? ` (${selOpt.Cost})` : '';
                return (
                  <div key={mod.Id}>
                    <span class="block text-[9px] font-mono font-semibold text-[var(--text-dim)] mb-0.5 uppercase tracking-wider">
                      {label}
                    </span>
                    <div class="w-full bg-[rgba(26,26,26,0.6)] border border-[rgba(51,51,51,0.15)] text-[var(--text)] text-xs font-mono px-2 py-1.5">
                      {optLabel}{cost}
                    </div>
                  </div>
                );
...
/** Render the stats section from a summary resource (identical to UnitEditorPanel). */
          ...
          <div>
            ...
            {uniqueWeapons.length > 0 && (
              <div class="px-3 py-2">
                ...
                ) : weaponView?.value === 'squad' && isInfantry && squadCount > 0 ? (
                  /* Squad view: individual members */
                    ...
                    {data.squadMembers.map((member, mi) => {
                      ...
                      return (
                        <SimpleTooltip key={`${member.Id}-${mi}`} text={tooltipLines}>
                          <div class="relative flex flex-col items-center bg-[rgba(26,26,26,0.4)] border border-[rgba(51,51,51,0.1)]">
                        ...
                        <div class="flex items-center gap-2">
                          ...
                          {isStatic && (
                            <SimpleTooltip text="Cannot shoot on the move">
                              <span class="text-[8px] font-mono font-bold uppercase tracking-wider text-[var(--accent)] bg-[rgba(70,151,195,0.08)] px-1 py-0.5 border border-[rgba(70,151,195,0.15)] flex-shrink-0">STATIC</span>
                            </SimpleTooltip>
                          )}
                            ...
                            <div key={ai} class={ai > 0 ? 'mt-1 pt-1 border-t border-[rgba(51,51,51,0.1)]' : 'mt-0.5'}>
                              ...
                              {aTraits.length > 0 && (
                                <div class="flex items-center gap-1.5 mt-0.5">
                                  {aTraits.map((tag) => (
                                    <SimpleTooltip key={tag} text={tag === 'TOP ATK' ? 'Attacks top armor' : tag === 'LASER' ? 'Laser guided munition' : tag}>
                                      <span class="text-[8px] font-mono font-bold uppercase tracking-wider text-[var(--accent)] bg-[rgba(70,151,195,0.08)] px-1.5 py-0.5 border border-[rgba(70,151,195,0.15)]">
                                        {tag}
                                      </span>
                                    </SimpleTooltip>
                                  ))}
...
export const ReadonlyUnitPanel = component$<ReadonlyUnitPanelProps>(
  ({ unit, arsenalCard, transportCard, locale, onClose$ }) => {
    const i18n = useI18n();
    ...
    const mainSummaryResource = useResource$<BuilderUnitSummary | null>(async ({ cleanup }) => {
    ...
    const tranSummaryResource = useResource$<BuilderUnitSummary | null>(async ({ cleanup }) => {
    ...
    const perUnitCost = baseCost + modCost;
    const unitTotal = perUnitCost * (unit.count ?? 1);
    ...
    const perTranCost = tranBaseCost + tranModCost;
    ...
    /* ═══════════════════════════ JSX ═══════════════════════════ */
    return (
      <div class="fixed inset-0 z-40 flex justify-end items-stretch pr-4 pt-16 pb-16 gap-2 pointer-events-none">
            ...
            <div class="flex items-center gap-3 px-3 py-4">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-[10px] font-mono text-[var(--text-dim)]">
                  <span class="text-[var(--accent)] font-bold">{perUnitCost}</span> {t(i18n, 'builder.editor.pts')}
                  {(unit.count ?? 1) > 1 && (
                    <>
                      {t(i18n, 'builder.unitEditor.eachSuffix')}
                      <span class="ml-2">{'\u00D7'}{unit.count}</span>
                    </>
                  )}
                  {modCost !== 0 && (
        ...
        {hasTransport && transportCard ? (
          <div class="w-[320px] max-h-[calc(100vh-2rem)] flex flex-col bg-gradient-to-b from-[var(--bg)] to-[rgba(26,26,26,0.95)] border border-[rgba(51,51,51,0.3)] shadow-2xl shadow-[0_0_25px_rgba(70,151,195,0.12)] pointer-events-auto relative overflow-hidden">
            ...
            {/* Header — static transport name (no dropdown) */}
            <div class="relative z-10 border-b border-[rgba(51,51,51,0.3)]">
              <div class="px-3 py-4">
                <div class="flex items-center gap-2">
                  {effectiveTranThumb && (
